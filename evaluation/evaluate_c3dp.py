...
def evaluate_proteins(model_checkpoint: str, device: torch.device, dtype: torch.dtype,
    ...
    model_state_dict, constructor_parameters = \
        load_model_checkpoint(model_checkpoint, device=device)
    print(f"Loading model from checkpoint with arguments: {constructor_parameters}\n")
    model = C3DPNet(**constructor_parameters)
    print(f"Loading model state_dict from checkpoint: {model_checkpoint}\n")
...
def main(args):
    ...
    scores = EVALUATION_DATASETS[args.dataset](model_checkpoint=args.model_checkpoint, device=device, dtype=dtype,
